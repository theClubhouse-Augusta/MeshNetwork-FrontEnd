...
export default class EventInformation extends Component {
  ...
  previousSponsors = () => {
    if (this.state.eventSponsors.length) {
      this.state.eventSponsors.forEach(sponsor => {
        ...
        this.setState(() => ({ selectedSponsors }));
      })
    }
  };
  ...
  handleOldChallengeTitle = (i, event) => {
    ...
    oldChallenges[i].challengeTitle = event.target.value;
  ...
  handleOldChallengeContent = (i, editorState) => {
    let oldChallenges = [...this.state.oldChallenges];
    oldChallenges[i].challengeContent = editorState;
    this.setState(() => ({
      oldChallenges
    }));
  }
  ...
  handleOldChallengeImage = (i, event) => {
    ...
    let file = event.target.files[0];
    reader.onloadend = () => {
      oldChallenges[i].challengeImage = file;
      oldChallenges[i].challengeImagePreview = reader.result;
      this.setState(() => ({
        oldChallenges
      }));
    };
  ...
  handleOldChallengeFile = (index, event) => {
    ...
    for (let i = 0; i < files.length; i++) {
      let fileData = { fileData: files[i], id: 0 };
      oldChallenges[index].challengeFiles.push(fileData);
      reader.onloadend = () => {
        this.setState(() => ({
          oldChallenges
        }));
  ...
  deleteOldFile = (i, j) => {
    let oldChallenges = [...this.state.challenges];
    oldChallenges[i].challengeFiles.splice(j, 1);
    this.setState(() => ({
      oldChallenges
    }));
  };
