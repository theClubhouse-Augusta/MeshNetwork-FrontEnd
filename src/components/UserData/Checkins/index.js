...
export class Checkins extends React.PureComponent {
  ...
  //   fetch(`https://testbean2-env.us-east-1.elasticbeanstalk.com/api/appearances/users/${this.props.match.params.id}/${date.get('month') + 1}/${date.get('year')}/${date.get('date')}`)
  ...
  onDatesChange = (startDate, endDate) => {
    ...
    if (startDate && endDate) {
      fetch(`https://testbean2-env.us-east-1.elasticbeanstalk.com/api/appearances/users/${this.props.match.params.id}/${startDate.get('month') + 1}/${startDate.get('year')}/${startDate.get('date')}/${endDate.get('month') + 1}/${endDate.get('year')}/${endDate.get('date')}`)
        .then(response => response.json())
        .then(({ users, error }) => {
          if (users) {
            this.setState(() => ({ users }));
