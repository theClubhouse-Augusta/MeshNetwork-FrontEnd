...
export class CustomerSignUps extends React.PureComponent {
  ...
  onDateChange = date => {
    ...
    if (date) {
      fetch(`https://innovationmesh.com/api/customers/signups/${this.props.match.params.id}/${date.get('month') + 1}/${date.get('year')}/${date.get('date')}`, {
  ...
  onDatesChange = (startDate, endDate) => {
    ...
    if (startDate && endDate) {
      fetch(`https://innovationmesh.com/api/customers/signups/${this.props.match.params.id}/${startDate.get('month') + 1}/${startDate.get('year')}/${startDate.get('date')}/${endDate.get('month') + 1}/${endDate.get('year')}/${endDate.get('date')}`, {
        headers: { Authorization: `Bearer ${localStorage['token']}` }
      })
        .then(response => response.json())
        .then(({ users, error }) => {
